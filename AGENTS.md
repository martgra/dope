...
uv run prek run --all-files          # Run linting checks using prek (compatible with pre-commit)
uv run prek run --all-files --hook-stage manual # Run all linting checks using prek
...
- Define custom exceptions in `dope/exceptions.py`
