...
def _set_doc_types() -> set[FileSuffix]:
    ...
    result = questionary.checkbox(message="Select doc file types.", choices=choices).ask()
...
def _set_exclude_folders(doc_root: Path) -> list[str]:
    ...
    if choices:
        result = questionary.checkbox(
            "Select folders to exclude from doc scan", choices=choices
        ).ask()
        return set(result) if result else set()
    return set()
...
def set_value(
    ...
    try:
        ...
        elif isinstance(old_value, frozenset):
            new_value = frozenset(value.split(","))
        else:
